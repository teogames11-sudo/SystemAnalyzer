"""
Обновляет version_file.txt перед сборкой EXE.
Использование: python scripts/update_version.py 1.2.3
"""
import sys
import re
...
def update_version_file(version: str):
    parts = version.split(".")
    if len(parts) != 3:
        print(f"Ошибка: ожидается формат X.Y.Z, получено '{version}'")
        sys.exit(1)
    ...
    print(f"version_file.txt обновлён до {version}")

    # Обновить app/version.py
    ...
    with open(vpath, "w", encoding="utf-8") as f:
        f.write(vc)
    print(f"app/version.py обновлён до {version}")


if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Использование: python scripts/update_version.py 1.2.3")
        sys.exit(1)
